...
class BisectingKMeans:
    def __init__(
        ...
        self._assign(data_points, middle_point_cls, kmeans_max_iter, kmeans_num_of_trys)
    ...
    def _bisect_data(self, data_points: list[DataPoint], data_points_index: list[int], parent_cluster_index: int):
        kmeans = KMeans(data_points, self._middle_point_cls, 2, self._kmeans_max_iter, self._kmeans_num_of_trys)
        ...
        for i in len(range(kmeans.result)):
            ...
            self._result[data_point_index] = next_cluster_index + kmeans.result[i]
        pass
